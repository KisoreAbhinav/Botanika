...
def main(argv: list[str] | None = None) -> int:
    ...
    with serve_dist() as url, sync_playwright() as playwright:
        ...
        try:
            ...
            page.get_by_role("button", name="Home").click()
            page.locator(".home-card").nth(1).click()
            page.wait_for_selector(".library", timeout=3000)
            page.get_by_role("button", name="Observation map", exact=True).click()
            page.wait_for_selector(".observation-map", timeout=3000)
            assert_fixed_pi_canvas(page)
            assert_persistent_masthead(page)
            page.screenshot(path=str(args.output / "library-map-800x480.png"))
            page.get_by_role("button", name="Vellore checklist", exact=True).click()
            page.wait_for_selector(".regional-checklist", timeout=3000)
            assert_fixed_pi_canvas(page)
            page.screenshot(path=str(args.output / "library-region-800x480.png"))
        finally:
            context.close()
        ...
        try:
            ...
            page.wait_for_selector(".observation-map", timeout=3000)
            assert_portrait_layout(page)
            page.screenshot(path=str(args.output / "library-map-browser-390x844.png"))
        finally:
            context.close()
        browser.close()
